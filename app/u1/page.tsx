...
// >>> SUBSTITUA TODO O SEU COMPONENTE POR ESTE <<<
...
export default function TargetIdentificationFlowPage() {
  ...
  // --- [NOVO] Lógica para montar o widget da Hotmart ---
  useEffect(() => {
    // Esta função só executa quando a Etapa 3 (resultados) é exibida
    if (step === 3) {
      // Verifica se a biblioteca da Hotmart já foi carregada e está disponível
      // A "any" é usada aqui porque 'checkoutElements' é injetado no window globalmente
      if (typeof (window as any).checkoutElements !== 'undefined') {
        console.log("Hotmart library found, mounting widget...");
        try {
          // Monta o widget de sales funnel no div com o id #hotmart-sales-funnel
  ...
  }, [step]); // O hook é re-executado toda vez que o 'step' muda
    ...
    >
        <div className="flex items-start justify-between">
            <div className="flex items-center gap-4 text-left">
                {profileImageUrl ? ( <img src={profileImageUrl} alt="profile" className="w-14 h-14 rounded-full object-cover filter grayscale" />) 
                ...
                <div>
                    ...
                    <p className="text-gray-400 text-sm">{profile.media_count} posts • {profile.follower_count} followers</p>
                </div>
            </div>
        ...
        {profile.biography && (<div className="border-t border-green-500/20 mt-3 pt-3 text-left"><p className="text-gray-300 text-sm">{profile.biography}</p></div>)}
    </div>
  );
  ...
  const renderInitialStep = () => (
    <>
      ...
      <div className="relative w-full">
        ...
        <Input type="text" placeholder="username" className="w-full bg-white border-2 border-black/20 text-black pl-12 h-14 text-base rounded-lg focus:border-pink-500 focus:ring-pink-500/50 shadow-inner" value={instagramHandle} onChange={(e) => handleInstagramChange(e.target.value)}/>
      </div>
  ...
  const renderResultsStep = () => (
    <div className="space-y-6 animate-fade-in">
      ...
      {/* O container para o widget da Hotmart. Ele é preenchido pelo script. */}
  ...
  return (
    <>
      {/* --- [NOVO] Carrega a biblioteca da Hotmart em segundo plano --- */}
      {/* O 'strategy="afterInteractive"' garante que não vai atrasar o carregamento da sua página. */}
      <Script
